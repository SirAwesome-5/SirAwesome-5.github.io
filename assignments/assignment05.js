        ...
        ticks: {
          ...
          callback: function (value, index, values) {
            if( value==10 || value==100 || value==1000 || value==10000 
              || value==100000  || value==1000000 || value == 10000000){
                return value / 1000 + "k";
            }
          }
        },
...
function loadContent() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 
      ...
      const lastCallDate = localStorage.getItem("Last Call Date");
      const currentCallDate = dayjs().valueOf();
      if (lastCallDate === null) lastCallDate = currentCallDate;
      localStorage.setItem("Last Call Date", currentCallDate);
      let diff = currentCallDate - lastCallDate; // Gives the difference in milliseconds, 86400000 millisecond = 24 hours
      
      if (localStorage.getItem("Ajax Call") === null || diff > 86400000) {
        covidJson = this.responseText;
        localStorage.setItem("Ajax Call", covidJson);
        covidJsObj = JSON.parse(covidJson);
      }
      else {
        covidJsObj = JSON.parse(localStorage.getItem("Ajax Call"));
      }
